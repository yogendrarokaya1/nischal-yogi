...
const hotelController = {
  addHotel: async (req, res) => {
    ...
    try {
      const hotelResult = await Hotel.create({ hotel_id, hotel_name, l_l, hotel_location, amenities, seller_email, description });
      const imagePromises = req.files.map((file) =>
        Hotel.addImage(hotel_id, file.path)
      );
  ...
  deleteListing: async (req, res) => {
    ...
    try {
      ...
      const hotelResult = await Hotel.delete(client, hotel_id, seller_email);
      if (hotelResult.rowCount === 0) {
        await client.query("ROLLBACK");
        return res.status(404).json({ message: "Listing not found!" });
      }
      await Room.deleteByHotelId(client, hotel_id);
      await Hotel.deleteImages(client, hotel_id);
